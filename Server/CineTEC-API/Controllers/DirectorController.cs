...
namespace CineTEC_API.Controllers
{
  ...
  public class DirectorController : ControllerBase
  {
    ...
    [HttpPost]
    public JsonResult Create(Director director)
    {
      string query = @"
          insert into director(peliid, nombredirector)
          values (@peliid, @nombredirector)
          ";
      DataTable table = new DataTable();
      string sqlDataSource = _configuration.GetConnectionString(cadenaDeConexion);
      NpgsqlDataReader myReader;
      using (NpgsqlConnection myCon = new NpgsqlConnection(sqlDataSource))
      {
        ...
        using (NpgsqlCommand myComand = new NpgsqlCommand(query, myCon))
        {
          myComand.Parameters.AddWithValue("@peliid", director.peliid);
          myComand.Parameters.AddWithValue("@nombredirector", director.nombredirector);
          myReader = myComand.ExecuteReader();
          table.Load(myReader);
          myReader.Close();
          myCon.Close();
    ...
    [HttpPut]
    public JsonResult Update(Director director)
    {
      string query = @"
          update director
          set peliid = @peliid,
