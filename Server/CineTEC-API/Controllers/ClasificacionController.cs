...
namespace CineTEC_API.Controllers
{
  ...
  public class ClasificacionController : ControllerBase
  {
    ...
    public ClasificacionController(IConfiguration configuration)
    {
    ...
    [HttpPost]
    public JsonResult Create(Clasificacion clasificacion)
    {
      string query = @"
          insert into clasificacion(peliid, descripcion)
          values (@peliid, @descripcion)
          ";
      DataTable table = new DataTable();
      string sqlDataSource = _configuration.GetConnectionString(cadenaDeConexion);
      NpgsqlDataReader myReader;
      using (NpgsqlConnection myCon = new NpgsqlConnection(sqlDataSource))
      {
        ...
        using (NpgsqlCommand myComand = new NpgsqlCommand(query, myCon))
        {
          myComand.Parameters.AddWithValue("@peliid", clasificacion.peliid);
          myComand.Parameters.AddWithValue("@descripcion", clasificacion.descripcion);
          myReader = myComand.ExecuteReader();
          table.Load(myReader);
          myReader.Close();
          myCon.Close();
    ...
    [HttpPut]
    public JsonResult Update(Clasificacion clasificacion)
    {
      string query = @"
