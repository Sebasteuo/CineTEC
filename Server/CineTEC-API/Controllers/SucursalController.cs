...
namespace CineTEC_API.Controllers
{
  ...
  public class SucursalController : ControllerBase
  {
    ...
    {
      string query = @"
          select codigosucursal, nombre, ubicacion, cantidadsalas
          from sucursal
          ";
    ...
    [HttpPost]
    public JsonResult Create(Sucursal sucursal)
    {
      string query = @"
          insert into sucursal(codigosucursal, nombre, ubicacion, cantidadsalas)
          values (@codigosucursal, @nombre, @ubicacion, @cantidadsalas)
          ";
      DataTable table = new DataTable();
      string sqlDataSource = _configuration.GetConnectionString(cadenaDeConexion);
      NpgsqlDataReader myReader;
      using (NpgsqlConnection myCon = new NpgsqlConnection(sqlDataSource))
      {
        ...
        using (NpgsqlCommand myComand = new NpgsqlCommand(query, myCon))
        {
          myComand.Parameters.AddWithValue("@codigosucursal", sucursal.codigosucursal);
          myComand.Parameters.AddWithValue("@nombre", sucursal.nombre);
          myComand.Parameters.AddWithValue("@ubicacion", sucursal.ubicacion);
          myComand.Parameters.AddWithValue("@cantidadsalas", sucursal.cantidadsalas);
          myReader = myComand.ExecuteReader();
          table.Load(myReader);
          myReader.Close();
          myCon.Close();
    ...
    [HttpPut]
    public JsonResult Update(Sucursal sucursal)
    {
      string query = @"
          update sucursal
