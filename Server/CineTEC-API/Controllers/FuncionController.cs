...
namespace CineTEC_API.Controllers
{
  ...
  public class FuncionController : ControllerBase
  {
    ...
    public FuncionController(IConfiguration configuration)
    {
    ...
    [HttpPut]
    public JsonResult Update(Funcion funcion)
    {
      string query = @"
          update funcion
    ...
    [HttpDelete("[action]/{peliid}")]
    public JsonResult DeleteByPelicula(string peliid)
    {
      string query = @"
          delete from funcion
          where peliid = @peliid
      ...
      using (NpgsqlConnection myCon = new NpgsqlConnection(sqlDataSource))
      {
        ...
        using (NpgsqlCommand myComand = new NpgsqlCommand(query, myCon))
        {
          myComand.Parameters.AddWithValue("@peliid", peliid);
          myReader = myComand.ExecuteReader();
          table.Load(myReader);
          myReader.Close();
          myCon.Close();
