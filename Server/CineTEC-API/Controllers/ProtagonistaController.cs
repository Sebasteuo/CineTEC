...
namespace CineTEC_API.Controllers
{
  ...
  public class ProtagonistaController : ControllerBase
  {
    ...
    public ProtagonistaController(IConfiguration configuration)
    {
    ...
    {
      string query = @"
          select peliid, nombreprotagonista
          from protagonista
          ";
    ...
    [HttpPost]
    public JsonResult Create(Protagonista protagonista)
    {
      string query = @"
          insert into protagonista(peliid, nombreprotagonista)
          values (@peliid, @nombreprotagonista)
          ";
      DataTable table = new DataTable();
      string sqlDataSource = _configuration.GetConnectionString(cadenaDeConexion);
      NpgsqlDataReader myReader;
      using (NpgsqlConnection myCon = new NpgsqlConnection(sqlDataSource))
      {
        ...
        using (NpgsqlCommand myComand = new NpgsqlCommand(query, myCon))
        {
          myComand.Parameters.AddWithValue("@peliid", protagonista.peliid);
          myComand.Parameters.AddWithValue("@nombreprotagonista", protagonista.nombreprotagonista);
          myReader = myComand.ExecuteReader();
          table.Load(myReader);
          myReader.Close();
          myCon.Close();
    ...
    [HttpPut]
    public JsonResult Update(Protagonista protagonista)
    {
      string query = @"
          update protagonista
          set peliid = @peliid,
